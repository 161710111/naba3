		...
		<div class="col-md-12">
			<div class="panel panel-primary">
			  ...
			  <div class="panel-body">
			  	<form action="{{ route('umroh.update',$umrohs->id) }}" method="post" >
        			...
        			{{ csrf_field() }}
			  		...
			  		<div class="form-group {{ $errors->has('kategorie_id') ? ' has-error' : '' }}">
			  			<label class="control-label">Kategori</label>	
			  			<select name="kategorie_id" class="form-control">
			  				@foreach($kategories as $data)
			  				<option value="{{ $data->id }}" {{ $selectedkategorie = $data->id ? 'selected="selected"' : '' }} >{{ $data->nama_kategori }}</option>
			  				@endforeach
			  			</select>
			  			@if ($errors->has('kategorie_id'))
                            <span class="help-block">
                                <strong>{{ $errors->first('kategorie_id') }}</strong>
                            </span>
                        @endif
			  		</div>
